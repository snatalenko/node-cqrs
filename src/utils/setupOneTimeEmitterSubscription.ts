 ...
 */
export default function setupOneTimeEmitterSubscription(
	emitter: IObservable,
	messageTypes: string[],
	...
	return new Promise(resolve => {
		...
		function filteredHandler(event: IEvent) {
			...
			logger?.log('debug', `'${event.type}' received, one-time subscription to '${messageTypes.join(',')}' removed`);
		...
		logger?.log('debug', `set up one-time ${filter ? 'filtered subscription' : 'subscription'} to '${messageTypes.join(',')}'`);
	});
}
